...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        validation_bottlenecks, validation_ground_truth, _ = (
            get_random_cached_bottlenecks(
                ...
                bottleneck_tensor))
        # validation step을 진행
        # TensorBoard를 위한 'merged' op을 이용해서 training summaries을 capture
    ...
    # 트레이닝 과정이 모두 끝
    # 따라서 이전에 보지 못했던 이미지를 통해 마지막 test 평가(evalution)을 진행
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--image_dir',
      type=str,
      default='/Users/Jihyun/Downloads/Recycle_test/mark',
      help='Path to folders of labeled images.'
  )
  parser.add_argument(
      '--output_graph',
      type=str,
      default='/tmp/tst/output_graph.pb',
      help='Where to save the trained graph.'
  )
  parser.add_argument(
      '--output_labels',
      type=str,
      default='/tmp/tst/output_labels.txt',
      help='Where to save the trained graph\'s labels.'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default='/tmp/tst/retrain_logs',
